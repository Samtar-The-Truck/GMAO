...
class MainWindow(QWidget):
    def __init__(self):
        ...
        del_eq_btn = QPushButton("Supprimer équipement")
        del_eq_btn.clicked.connect(self.delete_equipment)
        eq_layout.addWidget(self.eq_name)
        eq_layout.addWidget(self.eq_desc)
        eq_layout.addWidget(add_eq_btn)
        eq_layout.addWidget(del_eq_btn)
        ...
        del_task_btn = QPushButton("Supprimer tâche")
    ...
    def delete_equipment(self):
        ...
        if not item:
            return
        eq_id = item.data(0)
        cur.execute("DELETE FROM task WHERE equipment_id=?", (eq_id,))
